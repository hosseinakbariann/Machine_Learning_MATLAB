...
for a=1:size(find(test(:,3)==1))
    srta=find(test(:,3)==1);
    A(a,:)=test(srta(a,1),:);
end

for b=1:size(find(test(:,3)==2))
    srta=find(test(:,3)==2);
    B(b,:)=test(srta(b,1),:);
end

for c=1:size(find(test(:,3)==3))
    srta=find(test(:,3)==3);
    C(c,:)=test(srta(c,1),:);
end

for d=1:size(find(test(:,3)==4))
    srta=find(test(:,3)==4);
    D(d,:)=test(srta(d,1),:);
end

for e=1:size(find(test(:,3)==5))
    srta=find(test(:,3)==5);
    E(e,:)=test(srta(e,1),:);
end
for f=1:size(find(test(:,3)==6))
    srta=find(test(:,3)==6);
    F(f,:)=test(srta(f,1),:);
end

for g=1:size(find(test(:,3)==7))
    srta=find(test(:,3)==7);
    G(g,:)=test(srta(g,1),:);
end

for h=1:size(find(test(:,3)==8))
    srta=find(test(:,3)==9);
    H(h,:)=test(srta(h,1),:);
end

for i=1:size(find(test(:,3)==10))
    srta=find(test(:,3)==10);
    I(i,:)=test(srta(i,1),:);
end

for j=1:size(find(test(:,3)==11))
    srta=find(test(:,3)==11);
    J(j,:)=test(srta(j,1),:);
end

for k=1:size(find(test(:,3)==9))
    srta=find(test(:,3)==9);
   K(k,:)=test(srta(k,1),:);
end
